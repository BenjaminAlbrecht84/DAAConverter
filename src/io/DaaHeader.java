...
public class DaaHeader {
    ...
    public DaaHeader(File daaFile) {
        this.daaFile = daaFile;
        if (magicNumber == 0)
            load();
    }

    public DaaHeader(File daaFile, long offset) {
        this.daaFile = daaFile;
        this.offset = offset;
        if (magicNumber == 0)
            load();
    ...
    private void skip(InputStream is, long toSkip) {
        try {
            long n = toSkip;
            while (n > 0)
                n -= is.skip(n);
    ...
    public long getLocationOfBlockInFile(int blockIndex) {
        long location = headerSize + offset;
